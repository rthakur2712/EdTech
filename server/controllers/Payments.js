...
// console.log("userId",userId);
  if(!razorpay_order_id || !razorpay_payment_id || !razorpay_signature || !coursesId || !userId){
    return res.status(400).json({msg:"Payment failed due to invalid details"});
  }
  ...
  return res.status(400).json({msg:"Payment failed due to invalid signature"});
}
...
exports.sendPaymentSuccessEmail = async (req, res) => {
    ...
    try {
        ...
        await mailSender(
            enrolledStudent.email,
            `Payment Recieved`,
            `Payment of ${amount/100} has been recieved for order id ${orderId} and payment id ${paymentId}`
        )
    }
    catch (error) {
