        ...
        <div className='grid grid-cols-4 mx-auto px-[120px] py-[90px] '>
           {
            LearningGridArray.map((card,i)=>(
                <div
                key={i}
                className={`${i===0 && "col-span-2"} ${card.order %2 === 1?"bg-richblack-700":card.order % 2 ===0?"bg-richblack-800":"bg-transparent"}
                ${card.order === 3 && "col-start-2"}
                `}>
                    {
                        card.order<0?(<div className='w-[90%] flex flex-col gap-3'>
                            <h1 className='text-richblack-5 text-4xl'>{card.heading} <HighlightText text={card.highlightText}/></h1>
                            ...
                            <CTAbutton text={card.BtnText} link={card.BtnLink} active={true}
                            />
                            </div>
                        </div>):(<div className='p-8 flex flex-col gap-8 h-[250px]'>
