...
const onSubmit = (data) => {
  ...
  return (
    <div className='w-[665px] bg-richblack-800 p-6 text-richblack-5 rounded-md flex gap-6 flex-col'>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-5 ml-5'>
        ...
        <button onClick={()=>goBack()} className="bg-richblack-800 outline outline-richblack-700 w-fit px-6 py-3 rounded-md flex items-center gap-1 text-richblack-100"><MdKeyboardArrowLeft />Back</button>
        <button type='submit' disabled={loading} className='py-3 rounded-md px-6 bg-yellow-50 text-richblack-900'>Finish</button>
        </div>
